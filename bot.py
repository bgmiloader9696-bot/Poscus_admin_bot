#!/usr/bin/env python3
# POSCUS PREDICTER - WITH PORT BINDING FOR WEB SERVICE
...
async def show_all_devices(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔄 *Fetching devices...*", parse_mode="Markdown")
...
async def delete_device_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if device_id in keys:
        ...
        if result.get("ok"):
            await update.message.reply_text(f"✅ Deleted `{device_id}`", parse_mode="Markdown")
        else:
...
async def show_resellers(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔄 *Fetching resellers...*", parse_mode="Markdown")
    ...
    for rid, info in res.items():
        ...
        lines.append(f"{status} *{rid}*\n👤 {info.get('name','—')}\n📱 TG ID: `{info.get('telegram_id', 'N/A')}`\n🎫 {token_display} | Type: `{token_type}`\n💰 Remaining: `{remaining}`\n🔑 Token: `{info.get('token','')}`\n")
...
async def delete_reseller_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if reseller_id not in resellers:
        await update.message.reply_text(f"❌ Reseller `{reseller_id}` not found!", parse_mode="Markdown")
        return ConversationHandler.END
    
    result = api_post("/api/admin/reseller/delete", {"reseller_id": reseller_id})
    
    if result.get("ok"):
        await update.message.reply_text(f"✅ Deleted reseller `{reseller_id}`!", parse_mode="Markdown")
    else:
        await update.message.reply_text(f"❌ Failed: {result.get('msg', 'Error')}")
...
async def reseller_login_verify(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for rid, data in res.items():
        if data.get('token') == token and data.get('active'):
            ...
            await update.message.reply_text(
                f"✅ *Login Successful!*\n\n👤 Name: {data.get('name')}\n🆔 ID: {rid}\n📊 Type: `{token_type}`\n💰 Balance: `{token_display}`\n💎 Remaining: `{remaining}`\n\n⚠️ *Lifetime keys NOT allowed!*\n\n📱 Use 'Add Device' to create devices",
                parse_mode="Markdown",
                reply_markup=get_reseller_keyboard()
            )
            return ConversationHandler.END
...
async def reseller_add_device_get_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏳ *Select Expiry (No Lifetime):*", parse_mode="Markdown", reply_markup=get_reseller_expiry_keyboard())
...
async def reseller_add_device_get_expiry(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if "error" in stats:
        await query.edit_message_text(f"❌ {stats['error']}", parse_mode="Markdown")
        return ConversationHandler.END
