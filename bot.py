#!/usr/bin/env python3
# POSCUS PREDICTER - API TOGGLE (Complete Fake Mode when OFF)
...
# API TOGGLE (Admin can turn on/off)
api_enabled = True  # Default: ON
# ==============================================
...
# Server down message (same for everyone)
def get_server_down_msg():
    ms = random.randint(3000, 4000)
    return f"""⚠️ API Server is currently down!
...
# API functions - return server down message when OFF
def api_get(path, headers=None):
    if not is_api_enabled():
        print(f"⚠️ API is OFF - Returning down message for {path}")
        return {"error": get_server_down_msg()}
    
    try:
        r = requests.get(SERVER_URL + path, headers=headers or HEADERS_ADMIN, timeout=30)
        print(f"API GET {path}: Status={r.status_code}")
...
def api_post(path, data, headers=None):
    if not is_api_enabled():
        print(f"⚠️ API is OFF - Returning down message for POST {path}")
        return {"error": get_server_down_msg()}
    
    try:
        r = requests.post(SERVER_URL + path, headers=headers or HEADERS_ADMIN, json=data, timeout=30)
        print(f"API POST {path}: Status={r.status_code}, Response={r.text[:200]}")
        if r.status_code == 200:
            return r.json()
...
def api_delete(path, headers=None):
    if not is_api_enabled():
        print(f"⚠️ API is OFF - Returning down message for DELETE {path}")
        return {"error": get_server_down_msg()}
    
    try:
        r = requests.delete(SERVER_URL + path, headers=headers or HEADERS_ADMIN, timeout=30)
        print(f"API DELETE {path}: Status={r.status_code}, Response={r.text[:200]}")
        if r.status_code == 200:
            return r.json()
        return {"error": f"HTTP {r.status_code}", "msg": r.text}
    except Exception as e:
...
async def ping_server(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_api_enabled():
        await update.message.reply_text(get_server_down_msg())
        return
    ...
    try:
        ...
        ms = int((time.time() - s) * 1000)
        text = f"⚠️ API Server is currently down!\n\nServer: {SERVER_URL}\nResponse: {ms}ms\nStatus: 🔴 OFFLINE"
...
def get_admin_keyboard():
    ...
    return ReplyKeyboardMarkup([
        ["📋 All Devices", "➕ Add Device", "🗑️ Delete Device"],
        ["👥 Resellers", "➕ Add Reseller", "🗑️ Delete Reseller"],
        ["💰 Add Tokens", "📊 Stats", f"🔄 API Server {api_status}"],
        ["🌐 Ping Server", "🔑 Reseller Login", "🏠 Main Menu"]
    ], resize_keyboard=True)

def get_reseller_keyboard():
    return ReplyKeyboardMarkup([
        ["➕ Add Device", "🗑️ Delete Device", "🔍 Check Device", "💰 My Balance"],
        ["🌐 Ping Server", "🚪 Logout", "🏠 Main Menu"]
    ], resize_keyboard=True)
...
# ==================== WELCOME ====================
async def send_welcome(update: Update, role, name=None, balance=None, device_count=0):
    if not is_api_enabled():
        await update.message.reply_text(get_server_down_msg())
        return
    ...
    if role == "admin":
        ...
        if "error" in keys:
            await update.message.reply_text(keys["error"])
            return
...
Use buttons below to manage system."""
        ...
        if balance == -1:
            ...
            balance_display = f"{balance} tokens"
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(uid):
        ...
        await send_welcome(update, "admin")
        await update.message.reply_text("Use buttons below:", reply_markup=get_admin_keyboard())
        ...
        await send_welcome(update, "reseller", name, tokens_left, device_count)
        await update.message.reply_text("Use buttons below:", reply_markup=get_reseller_keyboard())
    
    else:
        ctx.user_data['role'] = 'normal'
        await send_welcome(update, "normal")
        await update.message.reply_text("Use buttons below:", reply_markup=get_normal_keyboard())

# ==================== API TOGGLE ====================
async def toggle_api_server(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    if not is_admin(uid):
        await update.message.reply_text("❌ Only admin can toggle API server!")
        return
    
    new_state = toggle_api()
    status = "ON ✅" if new_state else "OFF ❌"
    
    await update.message.reply_text(f"🔄 API Server is now {status}")
    
    # Update keyboard
    await update.message.reply_text("Keyboard updated!", reply_markup=get_admin_keyboard())
...
async def show_all_devices(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [f"📱 ALL DEVICES (Total: {len(keys)})\n{'-'*30}"]
...
async def add_device_get_expiry(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_api_enabled():
        ...
        await update.callback_query.edit_message_text(get_server_down_msg())
        return ConversationHandler.END
    ...
    if "error" in result:
        ...
        text = f"❌ Failed: {result.get('msg', result.get('error', 'Unknown'))}"
...
async def delete_device_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🔄 Deleting {device_id}...")
    
    result = api_delete(f"/api/admin/key/delete?key={device_id}")
    
    if "error" in result:
        await update.message.reply_text(result["error"])
    elif result.get("ok"):
        await update.message.reply_text(f"✅ Device {device_id} deleted successfully!")
    else:
        result2 = api_post("/api/admin/key/delete", {"key": device_id})
        if "error" in result2:
            await update.message.reply_text(result2["error"])
        elif result2.get("ok"):
            await update.message.reply_text(f"✅ Device {device_id} deleted successfully!")
        else:
            await update.message.reply_text(f"❌ Failed to delete: {result2.get('msg', 'Unknown error')}")
...
async def delete_reseller_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🔄 Deleting reseller {reseller_id}...")
    
    result = api_delete(f"/api/admin/reseller/delete?reseller_id={reseller_id}")
    
    if "error" in result:
        await update.message.reply_text(result["error"])
    elif result.get("ok"):
        await update.message.reply_text(f"✅ Reseller {reseller_id} deleted successfully!")
    else:
        result2 = api_post("/api/admin/reseller/delete", {"reseller_id": reseller_id})
        if "error" in result2:
            await update.message.reply_text(result2["error"])
        elif result2.get("ok"):
            await update.message.reply_text(f"✅ Reseller {reseller_id} deleted successfully!")
...
async def add_tokens_get_rid(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ctx.user_data['token_rid'] = update.message.text.strip()
...
async def add_tokens_get_amount(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if "error" in result:
        ...
        await update.message.reply_text(f"❌ Failed: {result.get('msg', result.get('error', 'Unknown'))}")
...
async def reseller_delete_device_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    result = api_delete(f"/api/reseller/key/delete?key={device_id}", headers=headers)
    ...
    if "error" in result:
        ...
        result2 = api_post("/api/reseller/key/delete", {"key": device_id}, headers=headers)
        if "error" in result2:
            await update.message.reply_text(result2["error"])
        elif result2.get("ok"):
            await update.message.reply_text(f"✅ Device {device_id} deleted successfully!")
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Check API status first for all non-admin operations
    if not is_api_enabled() and not is_admin(update.effective_user.id):
        # For non-admin users, show server down message for everything except logout
        if text == "🚪 Logout":
    ...
    if is_admin(update.effective_user.id) or ctx.user_data.get('role') == 'admin':
        if text == "📋 All Devices":
        ...
        if text == "📱 My Devices":
            ...
            headers = {"X-Reseller-Token": ctx.user_data['reseller_token']}
            stats = api_get("/api/reseller/stats", headers=headers)
            tokens_left = stats.get('tokens_left', 0) if "error" not in stats else 0
            name = stats.get('name', 'Reseller') if "error" not in stats else "Reseller"
            keys = api_get("/api/reseller/keys", headers=headers)
            device_count = len(keys) if isinstance(keys, dict) else 0
            await send_welcome(update, "reseller", name, tokens_left, device_count)
